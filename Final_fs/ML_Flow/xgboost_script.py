...
def train_and_evaluate(X_train, y_train, X_test, y_test, plot_name):
    ...
    with mlflow.start_run(run_name="XGBoost_Model") as run:
        model_name = f"XGBoost_{plot_name}"
        mlflow.set_tags({
            "model_name": model_name,
            "experiment_type": "XGBoost",
            "description": "Predicting Readmission with XGBoost"
        })

        # Initialize XGBoost model
        model = XGBClassifier(n_estimators=500, max_depth=15, scale_pos_weight=1, random_state=42)
